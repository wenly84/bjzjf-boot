...
public class CrmCustomerController {
    ...
    public void importTemplate(HttpServletResponse response) throws IOException {
        ...
        List<CrmCustomerImportExcelVO> list = Arrays.asList(
                CrmCustomerImportExcelVO.builder().name("智匠坊").industryId(1).level(1).source(1)
                        .mobile("15601691300").telephone("").qq("").wechat("").email("yaowenliang@hansi.site")
                        .areaId(null).detailAddress("").remark("").build(),
                CrmCustomerImportExcelVO.builder().name("汉斯大神").industryId(1).level(1).source(1)
                        .mobile("15601691300").telephone("").qq("").wechat("").email("yaowenliang@hansi.site")
                        .areaId(null).detailAddress("").remark("").build()
        );
        // 输出
